...
called the hidden layer, that has a nonlinear activation function (usually tanh or sigmoid).

This code is based upon the online theano tutorials."""
...
from libTheano.logistic_regression import LogisticRegression

import libVisualisation.visualisation as vis

import split_data as ml
...
def regularise_classification_model(L1_flag, L2_flag, Xpos, Xneg, ypos, yneg, alphas, var_name):
	...
	vis.plot_performance(alphas, error_val, error_train, 'error', 'regularisation parameter', 'logX',\
		'classification.'+var_name+'.regularisation.error.png')
	vis.plot_performance(alphas, P_val, P_train, 'precision', 'regularisation parameter', 'logX',\
		'classification.'+var_name+'.regularisation.precision.png')
	vis.plot_performance(alphas, R_val, R_train, 'recall', 'regularisation parameter', 'logX',\
		'classification.'+var_name+'.regularisation.recall.png')
	vis.plot_performance(alphas, F1_val, F1_train, 'F1 score', 'regularisation parameter', 'logX',\
...
def calculate_classification_learning_curves(L1_flag, L2_flag, best_alpha, Xpos, Xneg, ypos, yneg, var_name):
	...
	vis.plot_performance(proportion_of_samples, error_val_lc, error_train_lc, 'error', \
		'proportion of total training samples', 'linear',\
		'classification.'+var_name+'.learning_curves.error.png')
	vis.plot_performance(proportion_of_samples, P_val_lc, P_train_lc, 'precision', \
		'proportion of total training samples', 'linear',\
		'classification.'+var_name+'.learning_curves.precision.png')
	vis.plot_performance(proportion_of_samples, R_val_lc, R_train_lc, 'recall', \
		'proportion of total training samples', 'linear',\
		'classification.'+var_name+'.learning_curves.recall.png')
	vis.plot_performance(proportion_of_samples, F1_val_lc, F1_train_lc, 'F1 score', \
		'proportion of total training samples', 'linear',\
