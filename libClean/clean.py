...
Contains subroutines to:
        1) read in the data from file and store in pandas data frames
        2) scale the data appropriately
        3) put the data into matrices ready for machine learning applications
'''
...
def read_and_clean_data(input_dir):
    '''
    Read in raw data from file frame and stack into a single pandas data frame.
    Scale the data so that they have consistent units.
    '''
    #-------------------------------------------------------------
    # read data
...
def scale_variables(df):
    '''
    Scale the data so that all variables are on a per capital basis.
    '''
...
def set_up_pair_of_classification_matrices(df2):
    '''
    For the classification problem create a matrix of inputs and an associated 
    vector of outputs.
    The outputs are whether a countyr is in the OECD or not.
    '''
    Z = np.vstack(( \
...
def set_up_classification_matrices(df2):
    '''
    For the classification problem, create matrices of inputs leading to 
    positive and negative events and associated output vectors.
    The outputs are whether a countyr is in the OECD or not.
    '''
...
def set_up_regression_matrices(df2):
    '''
    For the regression problem, create a matrix of inputs and an associated vector of outputs.
    The vector of outputs is the life expectancy.
    '''
    Z = np.vstack(( \
        df2['death'].values, df2['birth'].values, df2['deathU5'].values, df2['gdpPC'].values,\
        df2['healthPC'].values, df2['healthPCGov'].values, df2['womenSchool'].values,\
